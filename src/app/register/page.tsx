    ...
    <>
      ...
      <main className="flex-grow min-h-screen flex items-center justify-center bg-cream py-32 px-6">
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
          ...
          <img src="/images/hero_bg.jpg" alt="Background" className="w-full h-full object-cover opacity-15 blur-xs" />
        </div>
        ...
        <div className="w-full max-w-md glass rounded-3xl p-8 md:p-10 shadow-luxury relative z-10">
          <div className="text-center mb-8">
            ...
            <h2 className="font-serif text-2xl text-forest font-semibold">Join EcoVoyage</h2>
            <p className="text-xs text-charcoal/60 mt-1.5 font-light">Register to start booking low-carbon travels</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs uppercase font-inter tracking-widest text-forest font-semibold block">Full Name</label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-forest/40" />
                <input
                  type="text"
                  value={name}
            ...
            >
              {loading ? 'Creating Member Profile...' : 'Register Account'}
            </button>
          </form>

          <div className="text-center mt-8 pt-6 border-t border-forest/5 text-xs text-charcoal/60">
            Already have a member account?{' '}
            <Link href="/login" className="text-gold font-semibold font-inter uppercase tracking-wider hover:text-yellow-600 ml-1">
              Sign In
            </Link>
          </div>
        </div>
